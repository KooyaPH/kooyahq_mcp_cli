...
test('README documents global GitHub install and configuration steps', () => {
  assert.match(readme, /npm install -g --install-links=true --ignore-scripts git\+https:\/\/github\.com\/KooyaPH\/kooyahq_mcp_cli\.git#v0\.2\.0/);
...
test('installation guides cover CLI and Codex MCP setup on Linux, macOS, and Windows', () => {
  for (const guide of [readme, installationGuide]) {
    ...
    assert.match(guide, /npm install -g --install-links=true --ignore-scripts git\+https:\/\/github\.com\/KooyaPH\/kooyahq_mcp_cli\.git#v0\.2\.0/);
