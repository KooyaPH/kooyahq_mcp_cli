...
export function localMcpDescriptor(dependencies: SetupDependencies): LocalMcpDescriptor {
  return {
...
export async function requireLocalMcpDescriptor(
  dependencies: SetupDependencies,
): Promise<LocalMcpDescriptor> {
...
export async function localMcpTargetCheck(
  dependencies: SetupDependencies,
): Promise<DoctorCheck> {
  try {
...
export function localMcpEntryMatches(value: unknown, descriptor: LocalMcpDescriptor): boolean {
...
export async function localMcpHandshakeCheck(
  dependencies: SetupDependencies,
): Promise<DoctorCheck> {
  const descriptor = localMcpDescriptor(dependencies);
  try {
...
export async function onlineStatusCheck(dependencies: SetupDependencies): Promise<DoctorCheck> {
  try {
...
export function projectGateCheck(): DoctorCheck {
  return {
...
export async function requireCommandRegistrationAbsent(
  dependencies: SetupDependencies,
  clientName: string,
  command: string,
  args: string[],
  isProvenAbsent: (result: CommandResult) => boolean,
  ...
  throw new ConfigError(
    `${clientName} cannot prove that kooyahq is absent; no changes were made. Complete manual registration after verifying the current client state: ${manualRegistration}.`,
  );
}
