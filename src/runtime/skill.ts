...
function commandSkillDocument(command: CommandSpec): Record<string, unknown> {
  ...
  if (command.fileInput) {
    parameters.push(
      { name: 'file', location: 'input', type: 'string', required: false },
      { name: 'stdin', location: 'input', type: 'switch', required: false },
      { name: 'format', location: 'input', type: 'string', required: false, choices: ['json', 'csv'] },
    );
  }
  return {
    schemaVersion: 1,
...
function parameterDocuments(
  ...
  return Object.entries(definitions).map(([name, definition]) => ({
    ...
    required: Boolean(
      command.requiredOptions?.includes(name)
