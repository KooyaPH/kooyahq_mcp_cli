...
function commandHelp(command) {
    ...
    const options = Object.entries(definitions)
        .map(([name, definition]) => formatOption(name, definition, command.requiredOptions?.includes(name) ?? false, command.exactlyOne?.some((group) => group.includes(name)) ?? false));
    ...
    if (command.confirmation)
        options.push('  --yes                  Skip the documented confirmation prompt.');
    ...
    const groups = [
        ...
        ...rangeDocuments(command.dateRange).map((range) => `  Date range: --${range.startOption} ${range.requireDistinctDates ? 'before' : 'through'} --${range.endOption}; maximum ${range.maxDays} inclusive calendar dates`),
    ...
    if (command.requireBody)
        groups.unshift('  At least one data option is required.');
...
function formatOption(name, definition, required, oneOf) {
